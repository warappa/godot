    ...
    public class ScriptMethodsGenerator : ISourceGenerator
    {
        ...
        )
        {
            ...
            if (godotClassNonStaticMethods.Length > 0)
            {
                ...
                source.Append("    public unsafe class ScriptMethodPointerHelper\n");
                source.Append("    {\n");
                foreach (var method in godotClassNonStaticMethods)
                {
                    GenerateScriptMethodPointerHelperMethod(symbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat), method, source);
                }
                source.Append("    }");
        ...
        )
        {
            ...
            source.Append("        .Register(MethodName.")
                ...
                .Append(", ")
                .Append($"ScriptMethodPointerHelper.CreateScriptMethod_{methodName}{method.ParamTypeSymbols.Length}())\n");
        }

        private static void GenerateScriptMethodPointerHelperMethod(
            string type,
            GodotMethodData method,
            StringBuilder source
        )
